...
import serviceAccountKey from "./mern-blog-fe15c-firebase-adminsdk-32j02-cfe8ad96cc.json" assert{type: "json"}
...
server.post("/signup", (req, res) => {
    ...
    if (!passwordRegex.test(password)) {
        return res.status(403).json({ "error": "Password should be 6 to 20 letter long with a numeric,1 lowercase and 1 uppercase letters" })
    }
...
server.post("/google-auth", async (req, res) => {
    ...
    getAuth()
        ...
        .then(async (decodedUser) => {
            ...
            let user = await User.findOne({ "personal_info.email": email }).select("personal_info.fullname personal_info.username personal_info.fullname personal_info.profile_img google_auth").then((u) => {
                return u || null
            })
                .catch(err => {
...
server.post('/create-blog',verifyJWT,(req,res) =>{
    ...
    if(id){
        ...
        .catch( err=>{
            return res.status(500).json({ error:err.mmessage })
        })
...
server.post("/get-blog",(req,res)=>{
    ...
    let incrimentVal = mode != 'edit' ? 1 : 0;

    Blog.findOneAndUpdate({ blog_id }, { $inc : { "activity.total_reads":incrimentVal }})
    .populate("author","personal_info.fullname personal_info.username personal_info.profile_img")
    .select("title des content banner activity publishedAt blog_id tags")
    .then(blog=>{

        User.findOneAndUpdate({ "personal_info.username":blog.author.personal_info.username },{
            $inc: {"account_info.total_reads":incrimentVal }
        })
        .catch(err=>{
            return res.status(500).json({ error: err.message })
        })
...
const deleteComments = ( _id ) => {
    ...
    .then(comment =>{
        ...
        Blog.findOneAndUpdate({ _id: comment.blog_id }, { $pull: { comments_id }, $inc:{ "activity.total_comments":-1 },"activity.total_parent_comments": comment.parent ? 0 : -1 })
        .then(blog =>{
